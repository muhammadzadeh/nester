...
@Injectable()
export class IsPrivateAttachmentConstraint implements ValidatorConstraintInterface {
	...
	async validate(value: any): Promise<boolean> {
		...
		const isContainPublicAttachment = attachments.findIndex((attachment) => attachment.isPublic()) > -1;
		return !isContainPublicAttachment && attachmentIds.length == attachments.length;
	}
